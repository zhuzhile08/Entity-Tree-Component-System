...
public:
	...
	using bucket_list = std::forward_list<bucket_type>;
	...
	constexpr iterator erase(const_iterator pos) noexcept {
		assert((pos != m_array.end()) && "lsd::UnorderedSparseSet::erase(): The end iterator was passed to the function!");
	...
	template <class K> [[nodiscard]] constexpr value_type& at(const K& key) {
		auto it = find(key);
		if (it == m_array.end()) throw std::out_of_range("lsd::UnorderedSparseSet::at(): Specified key could not be found in container!");
		return *it;
	}
...
private:
	...
	template <class K, class... Args> constexpr iterator basicEmplace(K&& key, Args&&... args) noexcept {
		...
		m_buckets[i].emplace_front(m_array.size());
		m_array.emplace_back(std::forward<K>(key), std::forward<Args>(args)...);
		rehashIfNecessary();
